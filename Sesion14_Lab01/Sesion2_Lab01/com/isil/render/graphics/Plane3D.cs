...
namespace Sesion2_Lab01.com.isil.render.graphics {
    public class Plane3D {
        ...
        public Plane3D(string path, float x, float y, float z, float size) {
            this.Initialize(path, x, y, z, size, 0f, 0f, TextureAddressMode.Clamp);
        }

        public Plane3D(string path, float x, float y, float z, float size, 
            float offsetTextCoordX, float offsetTextCoordY, TextureAddressMode mode) {

            this.Initialize(path, x, y, z, size, offsetTextCoordX, offsetTextCoordY, mode);
        }

        private void Initialize(string path, float x, float y, float z, float size,
            float offsetTextCoordX, float offsetTextCoordY, TextureAddressMode mode) {
            
            mTexture = new NTexture2D(NativeApplication.instance.Device, mode);
            mTexture.Load(path);
            ...
            mVertices[vertexOffset + 23] = 1f + offsetTextCoordY;
            ...
            mVertices[vertexOffset + 34] = 1f + offsetTextCoordX;
            mVertices[vertexOffset + 35] = 1f + offsetTextCoordY;
            ...
            mVertices[vertexOffset + 46] = 1f + offsetTextCoordX;
